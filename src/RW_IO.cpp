...
void readInputFile(struct parameters* param, int argc, char **argv)
{
    ...
    if (param->ns > 1)
        param->rhoINIT[1] = rhoINIT0.b;
    if (param->ns > 2)
        param->rhoINIT[2] = rhoINIT0.c;
    if (param->ns > 3)
        param->rhoINIT[3] = rhoINIT0.d;
    if (param->ns > 4)
...
void saveParameters(struct parameters* param)
{
    ...
    for (int is = 0; is < param->ns; is++)
        my_file << "rho init species " << is << " = " << param->rhoINIT[is] << std::endl;
...
void VTK_Write_Vectors(int cycle, struct grid *grd, struct EMfield* field, string cpuORgpu)
{
    ...
    my_fileE << "DIMENSIONS " << (nxn-3) << " " << (nyn-3) << " " << (nzn-3) << std::endl;
    ...
    my_file2 << "DIMENSIONS " << (nxn-3) << " " << (nyn-3) << " " << (nzn-3)  << std::endl;
...
void VTK_Write_Scalars(int cycle, struct grid *grd, struct interpDensSpecies* ids, struct interpDensNet* idn, string cpuORgpu)
{
    ...
    my_file << "DIMENSIONS " << (nxn-3) << " " << (nyn-3) << " " << (nzn-3) << std::endl;
    ...
    my_file1 << "DIMENSIONS " << (nxn-3) << " " << (nyn-3) << " " << (nzn-3) << std::endl;
